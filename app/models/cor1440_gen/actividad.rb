...
module Cor1440Gen
  class Actividad < ActiveRecord::Base
    ...
    def territorial_id
      oficina ? oficina.territorial_id : nil
    end

    attr_reader :territorial

    def territorial
      oficina ? oficina.territorial : nil
    end
    ...
    def valida_detallefinanciero_sin_repetidos
      pact = []
      detallefinanciero.map do |t|
        t.persona_ids.map do |per|
          if t.actividadpf_id
    ...
    def valida_actividad_marco_logico
      ...
      if controlador && controlador.params && controlador.params[:actividad] &&
        ...
        controlador.params[:actividad][:actividad_proyectofinanciero_attributes].each do |_l, v|
          ...
          cf = if v[:proyectofinanciero_id].to_i > 0
    ...
    scope :filtro_territorial, lambda { |tid|
      where(
        tid.count == 0 ? "TRUE" : "oficina_id IN (SELECT id FROM msip_oficina " \
    ...
    def casos_asociados
      lp = asistencia.pluck(:persona_id)
      if lp.count == 0
        lc = []
      else
    ...
    def poblacion_nuevos_ids
      idp += personas_asistentes_condicion do |a|
        Sivel2Gen::Victima.where(persona_id: a.persona_id).joins(:victimasjr)
          ...
          Sivel2Gen::Victima.find_by(persona_id: a.persona_id).caso.fecha
            .at_beginning_of_month >= fecha.at_beginning_of_month
      end
      idp.uniq!
      idp.join(",")
    end
    ...
    def poblacion_r_g(sexo, num)
      idp = personas_victimas_condicion do |v|
    ...
    def presenta(atr)
      ...
      when "beneficiarios_com_acogida"
        asistencia.select { |a| a.perfilorgsocial_id == 13 }.count
      ...
      when "num_afrodescendientes"
        cuenta_victimas_condicion do |v|
    ...
    def self.vista_reporte_extracompleto_excel(
      ...
      lt.add_worksheet do |hoja|
        ...
        hoja.add_row(
          [nil, nil, "Servicio Jesuita a Refugiados - Colombia"] +
                    [nil] * (numcol - 3),
          height: 30,
          style: estilo_titulo,
        )
