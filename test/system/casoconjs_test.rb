...
class CasoconjsTest < ApplicationSystemTestCase
  ...
  test "puede crear caso con familiar mínimo y 1 ubicación" do
    ...
    unless find_field("Nombres").visible?
      click_on "Añadir Víctima"
    ...
    unless find_link("Añadir Ubicación").visible?
      click_link "Ubicación"
    end
  ...
  test "puede crear caso con familiar mínimo, 2 ubicaciones, ref y desp" do
    ...
    # Refugio
    click_on "Refugio"
    unless find(:css, "#caso_casosjr_attributes_salida_id").visible?
      click_on "Refugio"
    end
    ...
    unless find_link("Añadir Desplazamiento").visible?
      click_on "Desplazamientos"
    end
    click_on "Añadir Desplazamiento"
    # page.save_screenshot('tmp/s-desp1.png')
    unless find_field("Fecha de Expulsión").visible?
      click_on "Añadir Desplazamiento"
    end
  ...
  test "puede crear caso con solicitante, p. resp y acto" do
    ...
    unless find_link("Añadir Presunto Responsable").visible?
      click_on "Presuntos Responsables"
    end
    click_on "Añadir Presunto Responsable"
    # page.save_screenshot('tmp/s-pr1.png')
    unless find_field("Presunto Responsable").visible?
      click_on "Añadir Presunto Responsable"
    end
    ...
    unless find_link("Añadir Causa/Antecedente").visible?
      click_on "Causas/Antecedentes"
    end
