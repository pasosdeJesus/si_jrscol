...
class Sivel2Sjr::ConteosController < ApplicationController
  ...
  def personas_arma_filtros
    personas_arma_filtros_sivel2_sjr
  end
  ...
  def personas_procesa_segun(que1, tablas1, where1, que3, tablas3, where3)
    personas_procesa_segun_omsjr(que1, tablas1, where1, que3, tablas3, where3)
  end
  ...
  def personas_vista_geo(que3, tablas3, where3)
    ...
    [
      "CREATE OR REPLACE MATERIALIZED VIEW #{personas_cons2} AS SELECT #{personas_cons1}.*,
  ...
  def vacios
    ...
    que3 << [
      "(SELECT nombre FROM public.sivel2_sjr_#{pContar} WHERE id=#{pContar}_id) AS atendido",
      @pque[pContar],
    ]
  ...
  def respuestas_que
    [
      ...
      "ayudaestado",
      "Respuestas y Derechos vulnerados",
    ]
  end
