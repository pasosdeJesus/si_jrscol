  ...
  class ValidarcasosController < ApplicationController
    ...
    def valida_nombres_beneficiarios_cortos
      ...
      res = reg
        ...
        .select([
          ...
          :nombres,
          :apellidos,
          "(SELECT sigla FROM msip_tdocumento WHERE msip_tdocumento.id=tdocumento_id LIMIT 1)",
          :numerodocumento,
    ...
    def validar_interno
      ...
      @validaciones << {
        ...
        cuerpo: val2[0][:cuerpo].map do |f|
          caso = nil
          actividad = nil
          casovictima = nil
          pf = nil
          if Sivel2Gen::AnexoCaso.where(anexo_id: f[0]).count > 0
            caso = Sivel2Gen::AnexoCaso.find_by(anexo_id: f[0]).caso_id
          elsif Sivel2Gen::AnexoVictima.where(anexo_id: f[0]).count > 0
            casovictima = Sivel2Gen::AnexoVictima.find_by(anexo_id: f[0]).victima.caso_id
          elsif Cor1440Gen::ActividadAnexo.where(anexo_id: f[0]).count > 0
            actividad = Cor1440Gen::ActividadAnexo.find_by(anexo_id: f[0]).actividad_id
          elsif Cor1440Gen::AnexoProyectofinanciero.where(anexo_id: f[0]).count > 0
            pf = Cor1440Gen::AnexoProyectofinanciero.find_by(anexo_id: f[0]).proyectofinanciero_id
          end
