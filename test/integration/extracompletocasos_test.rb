...
class ExtracompletocasosTest < ActionDispatch::IntegrationTest
  ...
  test "extracompleto sin job" do
    ...
    numdoc = 1
    numdoc += 1 while Msip::Persona.where(numerodocumento: numdoc).count > 0
    ...
    a = flash["notice"][%r{/generados/listado_extracompleto_de_casos-([0-9][0-9]*).ods}]

    assert_not a.nil?
    ruta = Rails.configuration.x.heb412_ruta.join(a.sub(%r{^/}, "") + "-0")
